...
async function scrapeCuloud(models) {
  let updated = 0
  try {
    const html = await fetchUrl('https://support.cucloud.cn/document/127/591/2357.html?id=2357&arcid=6518&lang=zh', 10000)
    if (!html) return
    const clean = html.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ')
    if (!clean.includes('按量计费')) return

    // 找到按量计费定价区域
    const idx = clean.indexOf('按量计费')
    if (idx < 0) return
    const section = clean.substring(idx, idx + 5000)

    // 提取各模型输入/输出价格（元/千tokens → ×1000 = 元/百万tokens）
    const pats = [
      { re: /DeepSeek-R1-Input[^0-9]*([\d.]+)/, model: 'DeepSeek-R1', field: 'i' },
      { re: /DeepSeek-R1-Output[^0-9]*([\d.]+)/, model: 'DeepSeek-R1', field: 'o' },
      { re: /DeepSeek-V3\.1-Input[^0-9]*([\d.]+)/, model: 'DeepSeek-V3.1', field: 'i' },
      { re: /DeepSeek-V3\.1-Output[^0-9]*([\d.]+)/, model: 'DeepSeek-V3.1', field: 'o' },
      { re: /DeepSeek-V3-Input[^0-9]*([\d.]+)/, model: 'DeepSeek-V3', field: 'i' },
      { re: /DeepSeek-V3-Output[^0-9]*([\d.]+)/, model: 'DeepSeek-V3', field: 'o' },
      { re: /DeepSeek-R1-Distill-Qwen-7B-Input[^0-9]*([\d.]+)/, model: 'DeepSeek-R1-Distill-Qwen-7B', field: 'i' },
      { re: /DeepSeek-R1-Distill-Qwen-7B-Output[^0-9]*([\d.]+)/, model: 'DeepSeek-R1-Distill-Qwen-7B', field: 'o' },
      { re: /DeepSeek-R1-Distill-Qwen-32B-Input[^0-9]*([\d.]+)/, model: 'DeepSeek-R1-Distill-Qwen-32B', field: 'i' },
      { re: /DeepSeek-R1-Distill-Qwen-32B-Output[^0-9]*([\d.]+)/, model: 'DeepSeek-R1-Distill-Qwen-32B', field: 'o' },
      { re: /DeepSeek-R1-Distill-Llama-8B-Input[^0-9]*([\d.]+)/, model: 'DeepSeek-R1-Distill-Llama-8B', field: 'i' },
      { re: /DeepSeek-R1-Distill-Llama-8B-Output[^0-9]*([\d.]+)/, model: 'DeepSeek-R1-Distill-Llama-8B', field: 'o' },
      { re: /Qwen2\.5-0\.5B-Instruct-Output[^0-9]*([\d.]+)/, model: 'Qwen2.5-0.5B-Instruct', field: 'o' },
      { re: /Qwen2\.5-1\.5B-Instruct-Output[^0-9]*([\d.]+)/, model: 'Qwen2.5-1.5B-Instruct', field: 'o' },
      { re: /Qwen2\.5-3B-Input[^0-9]*([\d.]+)/, model: 'Qwen2.5-3B-Instruct', field: 'i' },
      { re: /Qwen2\.5-3B-Output[^0-9]*([\d.]+)/, model: 'Qwen2.5-3B-Instruct', field: 'o' },
      { re: /Qwen2\.5-7B-Input[^0-9]*([\d.]+)/, model: 'Qwen2.5-7B-Instruct', field: 'i' },
      { re: /Qwen2\.5-7B-Output[^0-9]*([\d.]+)/, model: 'Qwen2.5-7B-Instruct', field: 'o' },
      { re: /Qwen2\.5-14B-Input[^0-9]*([\d.]+)/, model: 'Qwen2.5-14B-Instruct', field: 'i' },
      { re: /Qwen2\.5-14B-Output[^0-9]*([\d.]+)/, model: 'Qwen2.5-14B-Instruct', field: 'o' },
      { re: /Qwen2\.5-32B-Input[^0-9]*([\d.]+)/, model: 'Qwen2.5-32B-Instruct', field: 'i' },
      { re: /Qwen2\.5-32B-Output[^0-9]*([\d.]+)/, model: 'Qwen2.5-32B-Instruct', field: 'o' },
      { re: /Qwen2\.5-72B-Input[^0-9]*([\d.]+)/, model: 'Qwen2.5-72B-Instruct', field: 'i' },
      { re: /Qwen2\.5-72B-Output[^0-9]*([\d.]+)/, model: 'Qwen2.5-72B-Instruct', field: 'o' },
      { re: /Llama3-8B-Instruct[^0-9]*([\d.]+)/, model: 'Llama3-8B-Instruct', field: 'io' },
      { re: /Llama3-70B-Instruct[^0-9]*([\d.]+)/, model: 'Llama3-70B-Instruct', field: 'io' },
      { re: /QwQ_32B_Input[^0-9]*([\d.]+)/, model: 'QwQ-32B', field: 'i' },
      { re: /QwQ_32B_Output[^0-9]*([\d.]+)/, model: 'QwQ-32B', field: 'o' },
      { re: /bge_base_en_v1_5[^0-9]*([\d.]+)/, model: 'bge-base-en-v1.5', field: 'i' },
      { re: /bge_reranker_v2_m3[^0-9]*([\d.]+)/, model: 'bge-reranker-v2-m3', field: 'i' },
    ]
    for (const p of pats) {
      const m = section.match(p.re)
      if (m && parseFloat(m[1]) > 0) {
        const per1M = Math.round(parseFloat(m[1]) * 1000 * 100) / 100
        const matches = models.filter(mod => mod.n === p.model && mod.p === '中国联通')
        matches.forEach(mod => {
          if (p.field === 'io') { mod.i = per1M; mod.o = per1M }
          else mod[p.field] = per1M
          delete mod.stale
          updated++
        })
      }
    ...
    // Handle free models
    models.filter(m => m.n === 'DeepSeek-R1-Distill-Llama-70B' && m.p === '中国联通').forEach(m => {
      m.i = 0; m.o = 0; delete m.stale; updated++
    })
    // Handle Qwen2.5-0.5B/1.5B input = 0 (free input)
    models.filter(m => (m.n === 'Qwen2.5-0.5B-Instruct' || m.n === 'Qwen2.5-1.5B-Instruct') && m.p === '中国联通').forEach(m => {
      if (m.i === undefined || m.i === null) { m.i = 0; updated++ }
      delete m.stale
    })
  } catch (e) {}
  if (!updated) findModel(models, '').filter(m => m.p === '中国联通').forEach(m => m.stale = true)
  if (updated > 0) console.log('[中国联通] 更新 ' + updated + ' 个模型价格')
}
