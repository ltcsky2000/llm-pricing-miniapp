    ...
    for (const [modelName, headingId] of Object.entries(modelMap)) {
      ...
      if (inp && outp) {
        ...
        findModel(models, modelName).filter(m => m.p === 'Gemini').forEach(mod => {
          mod.i = ip; mod.o = op; if (cache) mod.cp = parseFloat(cache[1]); delete mod.stale; updated++
        })
      }
    }
  } catch (e) {}
